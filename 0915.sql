...
SELECT EMPNO, RPAD(SUBSTR(EMPNO, 1, 2), 4 ,'*')AS MASKING_EMPNO, 
    ENAME, RPAD(SUBSTR(ENAME, 1, 1), LENGTH(ENAME) ,'*')AS MASKING_
FROM EMP
WHERE LENGTH(ENAME) = 5;  
-- 2.
SELECT EMPNO, ENAME, SAL, TRUNC(SAL/21.5, 3) AS DAY_PAY,
    ROUND(SAL/(21.5*8), 2) AS TIME_PAY
FROM EMP;
-- 3. 
-- NEXT_DAY(기준일자, 찾을 요일) 기준 일자 다음에 오는 요일을 구하는 함수
SELECT EMPNO, ENAME, HIREDATE, 
    TO_CHAR(NEXT_DAY(ADD_MONTHS(HIREDATE, 3), 'MONDAY'), 'YYYY/MM/DD') AS 정직원,
